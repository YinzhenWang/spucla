...
def test(model, test_loader, loss_func):
    ...
    for images, labels in test_loader:
        ...
        print("test", loss.item(),images.size(0))
...
def train(model, train_loader, test_loader, loss_func, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in train_loader:
            ...
            print("train", epoch, loss.item(),images.size(0))
            loss_val += loss.item() * images.size(0)
...
def init_model(model_name, num_classes, is_fixed, use_pretrained):
    if (model_name == 'resnet'):
        ...
        model = resnet_ym.resnet50(pretrained=use_pretrained)  # 调用预训练的resnet101模型
        # 设置参数是否需要计算梯度
...
train_images = get_datasets(train_dir, input_size, is_train_data=True)
